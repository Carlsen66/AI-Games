...
function draw() {
  ...
  for (let i = activePegboards.length - 1; i >= 0; i--) {
    // activePegboards[i].nn_input = MasterPegBoard.nn_input;
    ...
    let col = rawprediction.splice(0, 7);
    let row = rawprediction.splice(0, 7);
    let mdir = rawprediction.splice(0, 4);
    let mrow = max(row);
    let mcol = max(col);
    let lm = max(mdir);
    for (pin_w = 0; pin_w < row.length; pin_w++) {
      if (mrow === row[pin_w]) break;
    }
    for (pin_h = 0; pin_h < col.length; pin_h++) {
      if (mcol === col[pin_h]) break;
    }
    for (pinmove = 0; pinmove < mdir.length; pinmove++) {
      if (lm === mdir[pinmove]) break;
    }
    // console.log(row)
    // console.log(col)
    // console.log(mdir)
    // console.log(rawprediction)
    // console.log(pin_h, pin_w, pinmove);
  ...
  ndate = new Date();
  ntime = ndate.getTime();
  dtime = (((ntime - ltime) / 1000) / 60);
  lasttime.html('Time:' + dtime.toFixed(2));
  // console.log('time:' + dtime.toFixed(4));
