...
function mutate(x) {
  if (random(1) < 0.1) {
    let offset = randomGaussian() * 0.05;
    let newx = x + offset;
    return newx;
  } else {
    return x;
...
class CreateBoard {
  constructor(brain, cols, rows) {
    ...
    if (brain instanceof NeuralNetwork) {
      ...
      this.brain = new NeuralNetwork(cols * rows, (cols * rows) * 5, 3);
    }
  }
  ...
  MovePeg(w, h, md) {
    ...
    if (this.pegs[w][h].wall === true && this.pegs[w][h].visible === true) {
      ...
      if (this.pegs[w][h].neighbors[md] && this.pegs[w][h].neighbors[md].wall === true && this.pegs[w][h].neighbors[md].visible === true) {
      
        if (tpeg.neighbors[md] && tpeg.neighbors[md].wall === false && tpeg.visible === true) {
